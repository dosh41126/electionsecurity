...
from blspy import PrivateKey, PublicKey, AugSchemeMPL, G2Element
from secretsharing import PlaintextToHexSecretSharer

import libp2p
from libp2p import new_node

from zokrates_pycrypto.proof import generate_proof, verify_proof
...
# ── IPFS CLIENT SETUP ────────────────────────────────────────────────────────
try:
    ipfs = ipfshttpclient.connect(config.get("IPFS_API", "/ip4/127.0.0.1/tcp/5001/http"))
...
provider_uri = config.get("WEB3_PROVIDER_URI", "http://127.0.0.1:8545")
web3 = Web3(Web3.HTTPProvider(provider_uri))
if web3.isConnected():
    abi_file       = path.join(bundle_dir, config["CONTRACT_ABI_PATH"])
    with open(abi_file) as f: contract_abi = json.load(f)
    contract_addr  = web3.toChecksumAddress(config["CONTRACT_ADDRESS"])
    contract       = web3.eth.contract(address=contract_addr, abi=contract_abi)
    logger.info("✅ Smart contract initialized")
else:
    logger.error(f"❌ Web3 provider not reachable at {provider_uri}")
    contract = None

# ── MISSING FUNCTION: GET CURRENT CANDIDATES ─────────────────────────────────
def get_current_candidates(election_id: str) -> list[str]:
    if contract is None:
        logger.error("get_current_candidates: contract not initialized")
        return []
    try:
        raw = contract.functions.getCandidates(election_id).call()
        out = []
        for c in raw:
            if isinstance(c, (bytes, bytearray)):
                out.append(c.decode('utf-8').rstrip('\x00'))
            else:
                out.append(str(c))
        return out
    except Exception as e:
        logger.error(f"get_current_candidates({election_id}) failed: {e}")
        return []

# ── OVERRIDE VERIFY CID ↔ HASH ───────────────────────────────────────────────
def verify_cid_hash(cid: str) -> bool:
    if ipfs is None or contract is None:
        logger.error("verify_cid_hash: IPFS or contract not set up")
        return False
    try:
        data       = ipfs.cat(cid)
        local_hash = hashlib.sha256(data).hexdigest()
        return contract.functions.isValidCid(cid, local_hash).call()
    except Exception as e:
        logger.error(f"[VerifyCID] {e}")
        return False
...
def init_ballot_db():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ballot_history (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id TEXT,
            election_id TEXT,
            cid TEXT,
...
# ── SAVE / LOAD BALLOT HISTORY ────────────────────────────────────────────────
def save_ballot_record(user_id: str, election_id: str, cid: str):
    ts = datetime.utcnow().isoformat() + "Z"
    with sqlite3.connect(DB_NAME) as conn:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO ballot_history (user_id,election_id,cid,timestamp) VALUES (?,?,?,?)",
            (user_id, election_id, cid, ts)
        )
        conn.commit()

def get_ballot_history(user_id: str) -> list[dict]:
    with sqlite3.connect(DB_NAME) as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT election_id,cid,timestamp FROM ballot_history WHERE user_id=? ORDER BY timestamp DESC",
            (user_id,)
        )
        rows = cur.fetchall()
    return [
        {"election_id": r[0], "cid": r[1], "timestamp": r[2]}
        for r in rows
    ]

# ── MULTICHOICE BALLOT SUBMISSION ─────────────────────────────────────────────
def submit_multichoice_ballot(election_id: str, selections: dict, user_id: str) -> str:
    ballot = {
        "election_id": election_id,
        "choices": selections,
        "timestamp": datetime.utcnow().isoformat() + "Z"
    }
    bjson = json.dumps(ballot)
    aad   = build_record_aad(user_id=user_id, source="ipfs", cls="Ballot")
    encrypted = crypto.encrypt(bjson, aad=aad)
    cid = ipfs.add_str(encrypted)
    save_ballot_record(user_id, election_id, cid)
    return cid

# ── FETCH & PREVIEW ENCRYPTED BALLOT ─────────────────────────────────────────
def fetch_and_preview_cid(cid: str) -> str:
    try:
        enc = ipfs.cat(cid).decode("utf-8")
        return crypto.decrypt(enc)
    except Exception as e:
        logger.error(f"[FetchPreview] {e}")
        return f"Error fetching/decrypting {cid}: {e}"
...
class App(customtkinter.CTk):
    ...
    def retrieve_past_interactions(self, user_input, result_queue):
        ...
        # ── LLM SYSTEM INTEGRITY AUDIT ───────────────────────────────────────────
        def perform_system_integrity_audit(self):
            try:
                prompt = (
                    "Perform a security audit of this election assistant system.\n"
                    "- Are blockchain CID links properly enforced?\n"
                    "- Are all ballot records encrypted?\n"
                    "- Is candidate syncing secure and verifiable?\n"
                    "- Can third parties verify audit QR codes?\n"
                    "- Are API and GUI inputs safe from injection?\n"
                    "Return a detailed audit summary and any suggested improvements."
                )
                result = llama_generate(prompt, self.client)
                self.response_queue.put({
                    'type': 'text',
                    'data': f"🛡️ System Audit:\n{result}"
                })
            except Exception as e:
                logger.error(f"[Audit] Failed LLM audit: {e}")

        def open_system_audit(self):
            # enqueue a notice, then run the audit in the background
            self.response_queue.put({
                'type': 'text',
                'data': "🔎 Running system integrity audit..."
            })
            self.executor.submit(self.perform_system_integrity_audit)
    ...
    # ── MULTICHOICE BALLOT WINDOW ───────────────────────────────────────────────
    def open_multichoice_window(self):
        win = customtkinter.CTkToplevel(self)
        win.title("Multichoice Ballot")
        win.geometry("400x600")

        # 1) Election ID entry
        customtkinter.CTkLabel(win, text="Election ID:").pack(pady=5)
        entry_eid = customtkinter.CTkEntry(win); entry_eid.pack(pady=5)

        # 2) Container for our checkboxes
        cb_vars = {}
        candidates_frame = customtkinter.CTkFrame(win)
        candidates_frame.pack(fill="both", expand=True, pady=10)

        # 3) Load button — calls get_current_candidates()
        def load_candidates():
            eid = entry_eid.get().strip()
            candidates = get_current_candidates(eid)

            # clear old
            for w in candidates_frame.winfo_children():
                w.destroy()
            cb_vars.clear()

            # render new
            customtkinter.CTkLabel(candidates_frame, text="Select candidates:").pack(anchor="w", padx=10)
            for name in candidates:
                var = tk.BooleanVar(value=False)
                customtkinter.CTkCheckBox(
                    candidates_frame, text=name, variable=var
                ).pack(anchor="w", padx=20, pady=2)
                cb_vars[name] = var

        customtkinter.CTkButton(win, text="Load Candidates", command=load_candidates).pack(pady=(0,10))

        # 4) Submit button
        def submit_ballot():
            eid    = entry_eid.get().strip()
            picks  = [n for n,v in cb_vars.items() if v.get()]
            cid    = submit_multichoice_ballot(eid, {"votes": picks}, self.user_id)
            self.response_queue.put({'type':'text','data':f"🗳️ Ballot submitted: {cid}"})
            win.destroy()

        customtkinter.CTkButton(win, text="Submit", command=submit_ballot).pack(pady=10)



    # ── BALLOT HISTORY VIEWER ───────────────────────────────────────────────────
    def view_ballot_history(self):
        history = get_ballot_history(self.user_id)
        if not history:
            self.response_queue.put({'type':'text','data':"No ballot history."})
            return

        win = customtkinter.CTkToplevel(self)
        win.title("Ballot History")
        win.geometry("500x400")
        for rec in history:
            txt = f"EID={rec['election_id']} | CID={rec['cid']} | {rec['timestamp']}"
            def cmd(cid=rec['cid']):
                return lambda: self.display_fetched_ballot(cid)
            customtkinter.CTkButton(win, text=txt, command=cmd()).pack(fill="x", pady=2, padx=5)

    # ── FETCH & PREVIEW CID WINDOW ──────────────────────────────────────────────
    def open_fetch_preview_window(self):
        win = customtkinter.CTkToplevel(self)
        win.title("Fetch & Preview CID")
        win.geometry("360x120")

        customtkinter.CTkLabel(win, text="Enter CID:").pack(pady=5)
        entry = customtkinter.CTkEntry(win); entry.pack(pady=5)
        def fetch():
            cid = entry.get().strip()
            content = fetch_and_preview_cid(cid)
            self.response_queue.put({'type':'text','data':f"📄 {cid}:\n{content}"})
            win.destroy()
        customtkinter.CTkButton(win, text="Fetch", command=fetch).pack(pady=5)

    def display_fetched_ballot(self, cid: str):
        content = fetch_and_preview_cid(cid)
        self.response_queue.put({'type':'text','data':f"📄 {cid}:\n{content}"})

    # ── VERIFY CID HASH WINDOW ──────────────────────────────────────────────────
    def open_verify_window(self):
        win = customtkinter.CTkToplevel(self)
        win.title("Verify CID")
        win.geometry("360x120")
        ...
        def verify():
            cid = entry.get().strip()
            ok  = verify_cid_hash(cid)
            status = "🟢 Valid" if ok else "🔴 Invalid"
            self.response_queue.put({'type':'text','data':f"Verification {cid}: {status}"})
            win.destroy()
        customtkinter.CTkButton(win, text="Verify", command=verify).pack(pady=5)
    ...
    def setup_gui(self):
        ...
        # ── NEW ELECTION FEATURES BUTTONS ─────────────────────────────────────
        self.btn_multichoice = customtkinter.CTkButton(
            self.sidebar_frame, text="🗳️ Submit Ballot", command=self.open_multichoice_window
        )
        self.btn_multichoice.grid(row=5, column=0, padx=20, pady=5)

        self.btn_history = customtkinter.CTkButton(
            self.sidebar_frame, text="🧾 Ballot History", command=self.view_ballot_history
        )
        self.btn_history.grid(row=6, column=0, padx=20, pady=5)

        self.btn_fetch = customtkinter.CTkButton(
            self.sidebar_frame, text="🌐 Fetch & Preview CID", command=self.open_fetch_preview_window
        )
        self.btn_fetch.grid(row=7, column=0, padx=20, pady=5)

        self.btn_verify = customtkinter.CTkButton(
            self.sidebar_frame, text="🧠 Verify CID Hash", command=self.open_verify_window
        )
        self.btn_verify.grid(row=8, column=0, padx=20, pady=5)

        # ── SYSTEM INTEGRITY AUDIT BUTTON ─────────────────────────────────────
        self.btn_audit = customtkinter.CTkButton(
            self.sidebar_frame, text="🛡️ Run System Audit", command=self.open_system_audit
        )
        self.btn_audit.grid(row=9, column=0, padx=20, pady=5)

        # ── PLACEHOLDER IMAGE ─────────────────────────────────────────────────
        self.image_label = customtkinter.CTkLabel(self.sidebar_frame)
        placeholder = tk.PhotoImage(width=140, height=140)
        placeholder.put(("gray",), to=(0, 0, 140, 140))
        ...
        self.text_box = customtkinter.CTkTextbox(
            ...
            border_width=0, height=360, width=50,
        ...
        self.input_textbox_scrollbar = customtkinter.CTkScrollbar(
            self.input_textbox_frame, command=self.input_textbox.yview
        )
